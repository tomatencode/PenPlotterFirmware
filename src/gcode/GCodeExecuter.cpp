...
void GCodeExecuter::handlePenUpDown(const std::string& cmd) {
    if (cmd == "M3") {
        _pen.down();
        delay(_runtimeSettings.penLiftTime_s() * 1000); // Small delay to allow pen to move down before starting motion
    } else if (cmd == "M5") {
        _pen.up();
        delay(_runtimeSettings.penLiftTime_s() * 1000); // Small delay to allow pen to move up before starting motion
    } else {
